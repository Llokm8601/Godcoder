...
/// Read the user's custom System Instructions (empty string when unset).
fn read_system_instructions(app_state: &AppState) -> String {
    app_state
        .db
        .get_setting(SYSTEM_INSTRUCTIONS_KEY)
        .ok()
        .flatten()
        .unwrap_or_default()
}
...
#[tauri::command]
pub async fn agent_get_system_instructions(app_state: State<'_, AppState>) -> Result<String, String> {
    Ok(read_system_instructions(&app_state))
}

#[tauri::command]
pub async fn agent_set_system_instructions(
    content: String,
...
/// Read the saved MCP server configs (empty when unset/corrupt).
fn read_mcp_servers(app_state: &AppState) -> Vec<agent::mcp::McpServerConfig> {
    app_state
        .db
...
fn write_mcp_servers(
    ...
    app_state.db.set_setting(MCP_SERVERS_KEY, &raw)
}

#[tauri::command]
pub async fn agent_list_mcp_servers(
    app_state: State<'_, AppState>,
) -> Result<Vec<agent::mcp::McpServerConfig>, String> {
    Ok(read_mcp_servers(&app_state))
}

/// Insert (when id empty) or update an MCP server config. Returns the saved row.
#[tauri::command]
pub async fn agent_save_mcp_server(
    server: agent::mcp::McpServerConfig,
    app_state: State<'_, AppState>,
) -> Result<agent::mcp::McpServerConfig, String> {
    if server.name.trim().is_empty() {
        return Err("Server name is required".to_string());
    }
    match server.transport {
        agent::mcp::McpTransport::Stdio if server.command.trim().is_empty() => {
            return Err("Command is required for the stdio transport".to_string());
        }
        agent::mcp::McpTransport::Http | agent::mcp::McpTransport::Sse
            if server.url.trim().is_empty() =>
        {
            return Err("Server URL is required for HTTP/SSE transports".to_string());
        }
        _ => {}
    }
    let mut servers = read_mcp_servers(&app_state);
    ...
    write_mcp_servers(&app_state, &servers)?;
    Ok(server)
}
...
/// Launch + connect to a server and list its tools, to validate the config.
#[tauri::command]
pub async fn agent_test_mcp_server(
    server: agent::mcp::McpServerConfig,
) -> Result<McpTestResult, String> {
    let conn = agent::mcp::connect_server(&server).await?;
    Ok(McpTestResult {
...
fn build_agent_config(
    ...
    extra_tools: Vec<Arc<dyn agent::tool::Tool>>,
) -> AgentConfig {
    ...
    // Inject user-defined System Instructions as a high-priority block right
    // after the static body so they steer every turn. Cached (ephemeral) since
    // they're stable across a session.
    if let Some(instructions) = user_instructions.map(str::trim).filter(|s| !s.is_empty()) {
        if let Some(blocks) = config.system_prompt.as_mut() {
            let block = agent::agent::prompt::SystemBlock {
                text: format!("# User's Custom Instructions\nThe user has provided the following instructions. Follow them carefully throughout this session.\n\n{instructions}\n"),
                cache_control: Some(agent::llm::types::CacheControl::ephemeral()),
            };
            // Insert after block 0 (the static mode body), before index/skills blocks.
            let pos = blocks.len().min(1);
...
/// A locally-synced cloud storage folder the user can start a session in.
#[derive(Serialize)]
pub struct CloudFolder {
    /// Stable id: "onedrive" | "googledrive" | "dropbox".
    pub provider: String,
    /// Human label shown on the quick-access chip.
    pub label: String,
...
fn push_cloud_dir(out: &mut Vec<CloudFolder>, provider: &str, label: &str, path: PathBuf) {
    let path_str = path.to_string_lossy().to_string();
    if path.is_dir() && !out.iter().any(|c| c.path == path_str) {
...
/// Detect locally-synced cloud storage folders (OneDrive, Google Drive, Dropbox)
/// so a session can be started inside one. Returns only folders that exist on
/// disk; never errors (an unconfigured provider is simply absent from the list).
#[tauri::command]
pub fn agent_detect_cloud_folders() -> Vec<CloudFolder> {
    let mut out = Vec::new();
    let home = home_dir();

    // OneDrive — env vars are set by the sync client; fall back to the default path.
    for var in ["OneDrive", "OneDriveConsumer", "OneDriveCommercial"] {
        if let Some(p) = std::env::var_os(var) {
            push_cloud_dir(&mut out, "onedrive", "OneDrive", PathBuf::from(p));
        }
    }
    if let Some(h) = &home {
        push_cloud_dir(&mut out, "onedrive", "OneDrive", h.join("OneDrive"));
    }

    // Dropbox — default folder, plus info.json which records relocated paths.
    if let Some(h) = &home {
        push_cloud_dir(&mut out, "dropbox", "Dropbox", h.join("Dropbox"));
    }
    let info_files = [
        std::env::var_os("APPDATA").map(|a| PathBuf::from(a).join("Dropbox").join("info.json")),
        home.as_ref().map(|h| h.join(".dropbox").join("info.json")),
    ];
    for info in info_files.into_iter().flatten() {
        if let Ok(txt) = std::fs::read_to_string(&info) {
            if let Ok(serde_json::Value::Object(accts)) = serde_json::from_str::<serde_json::Value>(&txt) {
                for (acct, v) in accts {
                    if let Some(p) = v.get("path").and_then(|p| p.as_str()) {
                        let label = if acct == "business" { "Dropbox (Business)" } else { "Dropbox" };
                        push_cloud_dir(&mut out, "dropbox", label, PathBuf::from(p));
                    }
                }
            }
        }
    ...
    // Google Drive — classic "Google Drive" folder, plus the virtual drive that
    // Google Drive for Desktop mounts (letter varies) containing "My Drive".
    if let Some(h) = &home {
        push_cloud_dir(&mut out, "googledrive", "Google Drive", h.join("Google Drive"));
    }
    for letter in 'D'..='K' {
        push_cloud_dir(
            &mut out,
            "googledrive",
            "Google Drive",
            PathBuf::from(format!("{letter}:\\My Drive")),
        );
    }
...
/// List local drive roots the user can start a session on. On Windows this is
/// every mounted drive letter (C:\, D:\, …); elsewhere it's the filesystem root.
#[tauri::command]
pub fn agent_list_local_drives() -> Vec<CloudFolder> {
    let mut out = Vec::new();
    if cfg!(windows) {
        for letter in 'A'..='Z' {
...
async fn run_agent_turn(
    ...
    // User-defined System Instructions (injected into the system prompt) and
    // tools discovered from configured MCP servers (registered for this session).
    let user_instructions = read_system_instructions(app_state);
    let mcp_servers = read_mcp_servers(app_state);
    let mcp_tools = if mcp_servers.iter().any(|s| s.enabled) {
        agent::mcp::connect_enabled(&mcp_servers).await
    } else {
        Vec::new()
    };
