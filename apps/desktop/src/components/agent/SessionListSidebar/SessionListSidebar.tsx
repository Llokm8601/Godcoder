...
import { Empty, Spin, Tooltip, Popconfirm, Dropdown } from "antd";
...
const NEW_SESSION_MODES: { mode: string; label: string; desc: string }[] = [
  { mode: "ask", label: "Ask", desc: "Read-only Q&A" },
...
export default function SessionListSidebar() {
  ...
  const { openSession, createInFolder, handleNewSession } = useSessionLauncher();
  ...
  // Menu of session modes (Ask / Plan / Coding / Freestyle). `folder` picks the
  // create path: a known folder creates in place; undefined prompts for one.
  const modeMenu = useCallback(
    (folder?: string): MenuProps => ({
      items: NEW_SESSION_MODES.map(({ mode, label, desc }) => {
        const Icon = MODE_ICON[mode] ?? MessageCircle;
        return {
          key: mode,
          label: (
          ...
          onClick: () => (folder ? createInFolder(folder, mode) : handleNewSession(mode)),
        };
      }),
    }),
    [createInFolder, handleNewSession],
  );
    ...
    return (
      <div className="h-full flex flex-col items-center justify-between py-3 bg-white dark:bg-dark-bg border-r border-gray-200 dark:border-dark-border">
        <div className="flex flex-col items-center gap-3">
          <Dropdown menu={modeMenu()} trigger={["click"]} placement="bottomLeft">
            <Tooltip title="New session" placement="right">
              <button type="button" aria-label="New session" className="w-9 h-9 flex items-center justify-center rounded-lg text-white bg-blue-600 hover:bg-blue-700">
        ...
        <div className="flex flex-col items-center gap-3">
          <Tooltip title="Settings" placement="right">
            <button type="button" aria-label="Settings" onClick={() => navigate("/settings")} className="w-9 h-9 flex items-center justify-center rounded-lg text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-dark-hover">
              <SettingsIcon className="w-4 h-4" />
            </button>
          </Tooltip>
          <Tooltip title="Expand sidebar" placement="right">
            <button type="button" aria-label="Expand sidebar" onClick={toggleSidebar} className="w-9 h-9 flex items-center justify-center rounded-lg text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-dark-hover">
            ...
            return (
              <div key={folder} className="mb-1.5">
                ...
                >
                  ...
                  <Dropdown menu={modeMenu(folder)} trigger={["click"]} placement="bottomRight">
