...
import { Segmented, Progress, Tooltip, Popover, App } from "antd";
...
export default function AgentInput({ sessionId, folderPath, agentName = "the agent" }: AgentInputProps) {
  ...
  // Latest finalized agent reply — fed to the voice controls for read-aloud.
  const lastAgentMessage = useAppStore((s) => {
    const msgs = s.agentThreads[sessionId]?.messages;
  ...
  // Dictation appends into the composer; voice-to-voice sends straight through.
  ...
  const voiceControls = (
    <VoiceControls
      ...
      replyText={lastAgentMessage?.text}
      replyId={lastAgentMessage?.id}
    />
  );
  ...
  const commandPickerDropdown =
    showCommandPicker && filteredCommands.length > 0 ? (
      <div
        ...
        className="command-picker-dropdown absolute bottom-full left-0 right-0 mb-1 mx-3 overflow-hidden border rounded-lg shadow-lg z-50"
      >
