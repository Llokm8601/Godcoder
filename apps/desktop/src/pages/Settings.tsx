...
import OllamaProvider from "@/components/settings/OllamaProvider";
...
export default function Settings() {
  ...
  const saveInstructions = async () => {
    ...
    try {
      await agentTauriService.setSystemInstructions(systemInstructions);
      themedMessage.success("System instructions saved");
    } catch {
      themedMessage.error("Failed to save instructions");
    } finally {
        ...
        {draft ? (
          <div className="flex flex-col gap-5 border border-[var(--border)] rounded-lg p-5 mb-6">
            <div className="text-sm font-medium text-[var(--text-primary)]">
              {isNew
                ...
                : `Edit ${providerName(draft)}`}
            </div>

            {draft.kind === "ollama" ? (
              <OllamaProvider draft={draft} setDraft={setDraft} />
            ) : (
              <>
            {!isBuiltin(draft) && (
              <div>
          ...
          <>
            ...
            <div className="flex flex-wrap gap-2 mb-8">
              <Button icon={<Plus className="w-4 h-4" />} onClick={startAddCompatible}>
                Add OpenAI-compatible provider
              </Button>
              <Button icon={<Plus className="w-4 h-4" />} onClick={startAddOllama}>
                Add Local LLM (Ollama)
              </Button>
            </div>
            ...
            <p className="text-sm text-[var(--text-secondary)] mb-3">
              Custom guidance prepended to the agent's system prompt for every session (coding style,
            ...
            <Input.TextArea
              ...
              placeholder="e.g. Always use TypeScript strict mode. Prefer functional React components. Never commit without running tests."
              className="mb-2"
            />
            ...
            <p className="text-sm text-[var(--text-secondary)] mb-4">
              Connect Model Context Protocol servers to give the agent extra tools (filesystem, GitHub,
              browsers, databases…). Enabled servers launch per session; their tools appear as
