...
export const agentTauriService = {
  ...
  async detectCloudFolders(): Promise<CloudFolder[]> {
    return invoke<CloudFolder[]>('agent_detect_cloud_folders');
  },

  /** List local drive roots (C:\, D:\, … on Windows; / elsewhere). */
  async listLocalDrives(): Promise<CloudFolder[]> {
  ...
  /** Probe whether the local Ollama server is reachable at `baseUrl`'s host. */
  async ollamaStatus(baseUrl: string): Promise<OllamaStatus> {
    return invoke<OllamaStatus>('ollama_status', { baseUrl });
  },

  /** List models installed in the local Ollama store. */
  async ollamaListModels(baseUrl: string): Promise<OllamaModel[]> {
    return invoke<OllamaModel[]>('ollama_list_models', { baseUrl });
  },

  /** Pull a model from the Ollama library or Hugging Face (`hf.co/…`). Streams `ollama:progress`. */
  async ollamaPull(baseUrl: string, model: string): Promise<void> {
    return invoke<void>('ollama_pull', { baseUrl, model });
  },

  /** Import a local GGUF file from disk into Ollama. Streams `ollama:progress`. */
  async ollamaImportGguf(baseUrl: string, name: string, path: string): Promise<void> {
    return invoke<void>('ollama_import_gguf', { baseUrl, name, path });
  },

  // ── System instructions (custom system prompt) ──────────────────────
  async getSystemInstructions(): Promise<string> {
    return invoke<string>('agent_get_system_instructions');
  },
  ...
  async listMcpServers(): Promise<McpServerConfig[]> {
    return invoke<McpServerConfig[]>('agent_list_mcp_servers');
  },

  async saveMcpServer(server: McpServerConfig): Promise<McpServerConfig> {
    return invoke<McpServerConfig>('agent_save_mcp_server', { server });
  },

  async deleteMcpServer(id: string): Promise<void> {
    return invoke<void>('agent_delete_mcp_server', { id });
  },
  ...
  async testMcpServer(server: McpServerConfig): Promise<McpTestResult> {
    return invoke<McpTestResult>('agent_test_mcp_server', { server });
  },
