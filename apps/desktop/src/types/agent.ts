...
export type SessionMode = 'ask' | 'plan' | 'coding' | 'freestyle';

/** A quick-access source folder: a synced cloud drive or a local drive root. */
export interface CloudFolder {
  provider: 'onedrive' | 'googledrive' | 'dropbox' | 'local';
  label: string;
  path: string;
}
...
/** Local Ollama runtime status (probe of the configured host). Mirrors Rust `OllamaStatus`. */
export interface OllamaStatus {
  running: boolean;
  version: string | null;
}

/** A model installed in the local Ollama store. Mirrors Rust `OllamaModel`. */
...
/** Streamed progress for a long-running Ollama pull/import (`ollama:progress` event). */
...
/** MCP transport: local child process (stdio) or remote HTTP/SSE. */
export type McpTransport = 'stdio' | 'http' | 'sse';

/** A configured MCP (Model Context Protocol) server. Mirrors Rust `McpServerConfig`. */
...
/** Result of testing an MCP server connection — the tools it advertises. */
